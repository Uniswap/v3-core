...
describe('LiquidityMath', () => {
  ...
  describe('#subDelta', () => {
    ...
    it('2**128 - 15 - 15 overflows', async () => {
      await expect(liquidityMath.subDelta(BigNumber.from(2).pow(128).sub(15), -15)).to.be.revertedWith('')
    })
    it('0 - 1 underflows', async () => {
      await expect(liquidityMath.subDelta(0, 1)).to.be.revertedWith('')
    })
    it('3 - 4 underflows', async () => {
      await expect(liquidityMath.subDelta(3, 4)).to.be.revertedWith('')
    })
    it('gas sub', async () => {
