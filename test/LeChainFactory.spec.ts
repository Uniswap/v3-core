...
describe('LeChainFactory', () => {
  ...
  const fixture = async () => {
    ...
    return (await factoryFactory.deploy()) as unknown as LeChainFactory
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('LeChainPool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('LeChainPool')
  ...
  describe('#enableFeeAmount', () => {
    ...
    it('enables pool creation', async () => {
      ...
      await createAndCheckPool([TEST_ADDRESSES[0], TEST_ADDRESSES[1]], 250 as unknown as FeeAmount, 15)
    })
  })
})
