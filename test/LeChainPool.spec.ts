...
import { TestLeChainSwapPay } from '../typechain/TestLeChainSwapPay'
...
import { TestLeChainReentrantCallee } from '../typechain/TestLeChainReentrantCallee'
...
describe('LeChainPool', () => {
  ...
  let pool: MockTimeLeChainPool

  let swapTarget: TestLeChainCallee
  ...
  describe('#burn', () => {
    ...
    /*it('does not clear the position fee growth snapshot if no more liquidity', async () => {
      ...
      await pool.connect(other.address).burn(minTick, maxTick, expandTo18Decimals(1))
      const {
        liquidity,
        tokensOwed0,
        tokensOwed1,
  ...
  async function initializeAtZeroTick(pool: MockTimeLeChainPool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#feeProtocol', () => {
    ...
    it('cannot be changed by addresses that are not owner', async () => {
      await expect(pool.connect(other.address).setFeeProtocol(6, 6)).to.be.reverted
    })
  ...
  describe('#setFeeProtocol', () => {
    ...
    it('can only be called by factory owner', async () => {
      await expect(pool.connect(other.address).setFeeProtocol(5, 5)).to.be.reverted
    })
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestLeChainReentrantCallee')
      ).deploy()) as unknown as TestLeChainReentrantCallee
  ...
  describe('fees overflow scenarios', async () => {
    ...
    /*it('two positions at the same snapshot', async () => {
      ...
      await pool.burn(minTick, maxTick, 0)
      await pool.connect(other.address).burn(minTick, maxTick, 0)
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestLeChainSwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestLeChainSwapPay')
      underpay = (await underpayFactory.deploy()) as unknown as TestLeChainSwapPay
