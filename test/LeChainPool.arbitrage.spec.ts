...
import { LeChainPoolSwapTest } from '../typechain/LeChainPoolSwapTest'
...
describe('LeChainPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('LeChainPoolSwapTest')
            const tester = (await testerFactory.deploy()) as unknown as LeChainPoolSwapTest
          ...
          let tester: LeChainPoolSwapTest
