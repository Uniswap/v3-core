...
describe('UniswapV2Factory', () => {
  ...
  async function createPair(tokens: [string, string]) {
    ...
    await expect(factory.createPair(...tokens)).to.be.revertedWith('UniswapV2: PAIR_EXISTS')
    await expect(factory.createPair(...tokens.slice().reverse())).to.be.revertedWith('UniswapV2: PAIR_EXISTS')
  ...
  it('createPair:gas', async () => {
    ...
    expect(receipt.gasUsed).to.eq(2645049)
  })
