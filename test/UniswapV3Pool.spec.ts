...
import {
  ...
  MAX_SQRT_RATIO,
  MIN_SQRT_RATIO,
  SAFE_ADVANCE_TIME_FOR_FEE_WITHDRAWAL,
...
describe('UniswapV3Pool', () => {
  ...
  describe('#mint', () => {
    ...
    describe('after initialization', () => {
      ...
      it('poke is not allowed on uninitialized position', async () => {
        ...
        // is and should be 'NP', hardhat is bugged
        ...
        expect(lastMintTime).to.eq(TEST_POOL_START_TIME)
  ...
  describe('#burn', () => {
    ...
    it('clears the position fee growth snapshot if no more liquidity', async () => {
      ...
      await pool.advanceTime(SAFE_ADVANCE_TIME_FOR_FEE_WITHDRAWAL)
  ...
  describe('#collect', () => {
    ...
    describe('penalty', () => {
      ...
      it('binds', async () => {
        await pool.burn(wallet.address, minTick, maxTick, 1)
      ...
      it('works with other positions', async () => {
        ...
        await pool.advanceTime(SAFE_ADVANCE_TIME_FOR_FEE_WITHDRAWAL)
        await pool.burn(wallet.address, minTick, 0, 0)

        let { feesOwed0, feesOwed1 } = await pool.positions(getPositionKey(wallet.address, minTick, 0))
        ...
        await pool.advanceTime(SAFE_ADVANCE_TIME_FOR_FEE_WITHDRAWAL)
        await pool.burn(wallet.address, minTick, 0, 0)
        ;({ feesOwed0, feesOwed1 } = await pool.positions(getPositionKey(wallet.address, minTick, 0)))
