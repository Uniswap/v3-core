...
describe('UniswapV3Pair', () => {
  ...
  describe('#initialize', () => {
    ...
    it('sets initial variables', async () => {
      ...
      expect((await pair.slot1()).liquidity).to.eq(1)
    })
  ...
  describe('#mint', () => {
    ...
    describe('after initialization', () => {
      ...
      it('0 liquidity mint does not affect protocol fee', async () => {
      ...
      it('positive liquidity mint does not affect protocol fee', async () => {
        await pair.setFeeProtocol(6)
        ...
        await mint(wallet.address, minTick + tickSpacing, maxTick - tickSpacing, expandTo18Decimals(10))
        expect(await pair.feeToFees0()).to.eq('49999999999999')
        expect(await pair.feeToFees1()).to.eq('4999999999999')
      })
  ...
  describe('miscellaneous setPosition tests', () => {
    ...
    it('mint to the right of the current price', async () => {
      ...
      const liquidityBefore = (await pair.slot1()).liquidity
      ...
      const liquidityAfter = (await pair.slot1()).liquidity
    ...
    describe('k (implicit)', () => {
      ...
      describe('post initialized', () => {
        ...
        it('updates correctly when exiting range', async () => {
          const kBefore = (await pair.slot1()).liquidity
          ...
          const kAfterSwap = (await pair.slot1()).liquidity
  ...
  describe('#feeProtocol', () => {
    ...
    it('is initially set to 0', async () => {
      expect((await pair.slot1()).feeProtocol).to.eq(0)
    })
    ...
    async function swapAndGetFeesOwed(swapAmount: BigNumberish, zeroForOne: boolean, poke: boolean) {
    ...
    it('position owner gets full fees when protocol fee is off', async () => {
      const { token0Fees, token1Fees } = await swapAndGetFeesOwed(expandTo18Decimals(1), true, true)
    ...
    it('swap fees accumulate as expected (0 for 1)', async () => {
      let token0Fees
      let token1Fees
      ;({ token0Fees, token1Fees } = await swapAndGetFeesOwed(expandTo18Decimals(1), true, true))
      expect(token0Fees).to.eq('599999999999999')
      expect(token1Fees).to.eq(0)
      ;({ token0Fees, token1Fees } = await swapAndGetFeesOwed(expandTo18Decimals(1), true, true))
    ...
    it('swap fees accumulate as expected (1 for 0)', async () => {
      let token0Fees
      let token1Fees
      ;({ token0Fees, token1Fees } = await swapAndGetFeesOwed(expandTo18Decimals(1), false, true))
    ...
    describe('#collect', () => {
      it('returns 0 if no fees', async () => {
        await pair.setFeeProtocol(6)
        const { amount0, amount1 } = await pair.callStatic.collectProtocol(wallet.address)
      ...
      it('can collect fees', async () => {
        ...
        await swapAndGetFeesOwed(expandTo18Decimals(1), true, true)
        ...
        await expect(pair.collectProtocol(other.address))
          .to.emit(token0, 'Transfer')
    ...
    it('fees collected by feeTo are only fees collected when fee is on', async () => {
      await swapAndGetFeesOwed(expandTo18Decimals(1), true, true)
    ...
    it('fees collected by lp after two swaps with intermediate withdrawal', async () => {
      ...
      // the fee to fees account for fees even though it was not poked
  ...
  it('tick transition cannot run twice if zero for one swap ends at fractional price just below tick', async () => {
    ...
    await initialize(p0)
    expect((await pair.slot1()).liquidity, 'current pair liquidity is 1').to.eq(1)
    ...
    expect((await pair.slot1()).liquidity, 'current pair liquidity is now liquidity + 1').to.eq(liquidity.add(1))

    await mint(wallet.address, -24082, -24081, liquidity)
    expect((await pair.slot1()).liquidity, 'current pair liquidity is still liquidity + 1').to.eq(liquidity.add(1))
    ...
    expect((await pair.slot1()).liquidity, 'pair has run tick transition and liquidity changed').to.eq(
