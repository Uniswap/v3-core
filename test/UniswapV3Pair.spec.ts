...
describe('UniswapV3Pair', () => {
  ...
  describe('miscellaneous setPosition tests', () => {
    ...
    it('collect fees within the current price after swap', async () => {
      ...
      await mint(walletAddress, lowerTick, upperTick, 0) // poke to update fees
      await pair.withdraw(lowerTick, upperTick, walletAddress, constants.MaxUint256, constants.MaxUint256)
    ...
    describe('k (implicit)', () => {
      ...
      describe('post initialized', () => {
        ...
        it('updates correctly when exiting range', async () => {
          const kBefore = await pair.slot1()
          ...
          const kAfterSwap = await pair.slot1()
  ...
  describe('#feeTo', () => {
    ...
    async function swapAndGetFeesOwed(
      ...
      if (!suppressPoke) {
        await mint(walletAddress, MIN_TICK, MAX_TICK, 0)
      }
    ...
    describe('#collect', () => {
      ...
      it('can collect fees', async () => {
        ...
        await mint(walletAddress, MIN_TICK, MAX_TICK, 0) // poke to update fees
        await pair.withdraw(MIN_TICK, MAX_TICK, walletAddress, constants.MaxUint256, constants.MaxUint256)
    ...
    it('fees collected by lp after two swaps with intermediate withdrawal', async () => {
      ...
      await mint(walletAddress, MIN_TICK, MAX_TICK, 0) // poke to update fees
      await expect(pair.withdraw(MIN_TICK, MAX_TICK, walletAddress, constants.MaxUint256, constants.MaxUint256))
        .to.emit(token0, 'Transfer')
  ...
  it('tick transition cannot run twice if zero for one swap ends at fractional price just below tick', async () => {
    ...
    await initialize(p0)
    expect(await pair.slot1(), 'current pair liquidity is 1').to.eq(1)
    ...
    expect(await pair.slot1(), 'current pair liquidity is now liquidity + 1').to.eq(liquidity.add(1))

    await mint(walletAddress, -24082, -24081, liquidity)
    expect(await pair.slot1(), 'current pair liquidity is still liquidity + 1').to.eq(liquidity.add(1))
    ...
    expect((await pair.slot0()).sqrtPriceCurrent._x, 'pair price is still on the p0 boundary').to.eq(p0.sub(1))
    expect(await pair.slot1(), 'pair has run tick transition and liquidity changed').to.eq(liquidity.mul(2).add(1))
