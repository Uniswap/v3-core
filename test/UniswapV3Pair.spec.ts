...
describe('UniswapV3Pair', () => {
  ...
  describe('#mint', () => {
    it('fails if not initialized', async () => {
      await expect(pair.mint(-1, 1, 0)).to.be.revertedWith('UniswapV3Pair::mint: pair not initialized')
    })
    describe('after initialization', () => {
      ...
      describe('failure cases', () => {
        ...
        it('fails if tickLower less than min tick', async () => {
          await expect(pair.mint(MIN_TICK - 1, 1, 0)).to.be.revertedWith(
            'UniswapV3Pair::mint: tickLower cannot be less than min tick'
          )
        })
        it('fails if tickUpper greater than max tick', async () => {
          await expect(pair.mint(-1, MAX_TICK + 1, 0)).to.be.revertedWith(
            'UniswapV3Pair::mint: tickUpper cannot be greater than max tick'
          )
        })
        it('fails if cannot transfer', async () => {
          await expect(pair.mint(MIN_TICK + 1, MAX_TICK - 1, 100)).to.be.revertedWith(
            'TransferHelper::transferFrom: transferFrom failed'
          )
        })
        it('fails if called with 0 amount', async () => {
          await expect(pair.mint(MIN_TICK + 1, MAX_TICK - 1, 0)).to.be.revertedWith(
            'UniswapV3Pair::mint: amount must be greater than 0'
          )
        })
        it('fails if liquidityDelta exceeds the max', async () => {
          await expect(pair.mint(MIN_TICK + 1, MAX_TICK - 1, MAX_LIQUIDITY_GROSS_PER_TICK.add(1))).to.be.revertedWith(
      ...
      describe('success cases', () => {
        ...
        describe('below current price', () => {
          it('transfers token0 only', async () => {
            await expect(pair.mint(-231, 0, 10000))
              .to.emit(token0, 'Transfer')
          ...
          it('works for max tick', async () => {
            await expect(pair.mint(-231, MAX_TICK, 10000))
              .to.emit(token0, 'Transfer')
          ...
          it('removes liquidity from liquidityGross', async () => {
            await pair.mint(-231, 0, 100)
            await pair.mint(-231, 0, 40)
            await pair.burn(-231, 0, 90)
          ...
          it('only clears the tick that is not used at all', async () => {
            await pair.mint(-231, 6, 100)
            await pair.mint(-53, 6, 250)
            await pair.burn(-231, 6, 100)
        ...
        describe('including current price', () => {
          ...
          it('initializes lower tick', async () => {
            await pair.mint(MIN_TICK + 1, MAX_TICK - 1, 100)
          ...
          it('removing works', async () => {
            await pair.mint(MIN_TICK + 1, MAX_TICK - 1, 100)
            await pair.burn(MIN_TICK + 1, MAX_TICK - 1, 100)
            expect(await token0.balanceOf(pair.address)).to.eq(10001)
            expect(await token1.balanceOf(pair.address)).to.eq(1002)
          })

          it('gas', async () => {
            await snapshotGasCost(pair.mint(MIN_TICK + 1, MAX_TICK - 1, 100))
          })
        })

        describe('above current price', () => {
          it('transfers token1 only', async () => {
            await expect(pair.mint(-500, -233, 10000))
              .to.emit(token1, 'Transfer')
          ...
          it('works for min tick', async () => {
            await expect(pair.mint(MIN_TICK, -233, 10000))
              .to.emit(token1, 'Transfer')
          ...
          it('removing works', async () => {
            await pair.mint(-500, -233, 10000)
            await pair.burn(-500, -233, 10000)
            expect(await token0.balanceOf(pair.address)).to.eq(10000)
            expect(await token1.balanceOf(pair.address)).to.eq(1002)
          })

          it('gas', async () => {
            await snapshotGasCost(await pair.mint(-500, -233, 10000))
          })
        })
      })
    })
  ...
  describe('post-initialize for low fee', () => {
    ...
    it('swap0For1 large swap crossing several initialized ticks', async () => {
      ...
      await pair.mint(-4, -2, expandTo18Decimals(1))
      await pair.mint(-8, -3, expandTo18Decimals(1))

      await expect(pair.swap0For1(expandTo18Decimals(1), walletAddress, '0x'))
        .to.emit(token1, 'Transfer')
        .withArgs(pair.address, walletAddress, '684358904605133184')
    })
    ...
    it('swap1For0 large swap crossing several initialized ticks', async () => {
      ...
      await pair.mint(2, 4, expandTo18Decimals(1))
      await pair.mint(3, 8, expandTo18Decimals(1))
  ...
  describe('limit orders', () => {
    ...
    it('selling 1 for 0 at tick 0 thru 1', async () => {
      await expect(pair.mint(0, 1, expandTo18Decimals(1)))
        .to.emit(token0, 'Transfer')
        .withArgs(walletAddress, pair.address, '4962809790010865')
      // somebody takes the limit order
      await pair.swap1For0(expandTo18Decimals(2), otherAddress, '0x')
      await expect(pair.burn(0, 1, expandTo18Decimals(1)))
        .to.emit(token1, 'Transfer')
        .withArgs(pair.address, walletAddress, '4987562112089026')
    })
    it('selling 0 for 1 at tick 0 thru -1', async () => {
      await expect(pair.mint(-1, 0, expandTo18Decimals(1)))
        .to.emit(token1, 'Transfer')
        .withArgs(walletAddress, pair.address, '4962809790010865')
      // somebody takes the limit order
      await pair.swap0For1(expandTo18Decimals(2), otherAddress, '0x')
      await expect(pair.burn(-1, 0, expandTo18Decimals(1)))
        .to.emit(token0, 'Transfer')
        .withArgs(pair.address, walletAddress, '4987562112089027')
    })
    ...
    describe('tickSpacing = 12', () => {
      ...
      describe('post initialize', () => {
        ...
        it('swapping across gaps works in 1 for 0 direction', async () => {
          ...
          await pair.mint(1200, 1212, liquidityAmount)
          await pair.swap1For0(expandTo18Decimals(1), walletAddress, '0x')
          await expect(pair.burn(1200, 1212, liquidityAmount))
            .to.emit(token0, 'Transfer')
            .withArgs(pair.address, walletAddress, '30563825225600')
            .to.emit(token1, 'Transfer')
            .withArgs(pair.address, walletAddress, '996999999999999604')
          expect(await pair.tickCurrent()).to.eq(1202)
        })
        it('swapping across gaps works in 0 for 1 direction', async () => {
          const liquidityAmount = expandTo18Decimals(1).div(4)
          await pair.mint(-1212, -1200, liquidityAmount)
          await pair.swap0For1(expandTo18Decimals(1), walletAddress, '0x')
          await expect(pair.burn(-1212, -1200, liquidityAmount))
            .to.emit(token0, 'Transfer')
            .withArgs(pair.address, walletAddress, '996999999999998206')
            .to.emit(token1, 'Transfer')
            .withArgs(pair.address, walletAddress, '30563825225600')
