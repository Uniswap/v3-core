...
describe('UniswapV3Pair', () => {
        ...
        describe('below current price', () => {
          it('transfers token0 only', async () => {
            await expect(pair.setPosition(-23028, 0, 10000))
              .to.emit(token0, 'Transfer')
              .withArgs(walletAddress, pair.address, 21623)
            expect(await token0.balanceOf(pair.address)).to.eq(10000 + 21623)
  ...
  describe('post-initialize for low fee', () => {
    ...
    it('swap0For1 large swap crossing several initialized ticks', async () => {
      ...
      await pair.setPosition(-398, -199, expandTo18Decimals(1))
      await pair.setPosition(-796, -299, expandTo18Decimals(1))

      await expect(pair.swap0For1(expandTo18Decimals(1), walletAddress, '0x'))
        .to.emit(token1, 'Transfer')
        .withArgs(pair.address, walletAddress, '684344452370530239')
    })
    ...
    it('swap1For0 large swap crossing several initialized ticks', async () => {
      ...
      await pair.setPosition(199, 398, expandTo18Decimals(1))
      await pair.setPosition(299, 796, expandTo18Decimals(1))
  ...
  describe('post-initialize at medium fee', () => {
    ...
    it('swap0For1 to tick -1k', async () => {
      ...
      await expect(pair.swap0For1(amount0In, walletAddress, '0x'))
        .to.emit(token1, 'Transfer')
        .withArgs(pair.address, walletAddress, '94965947516311847')
    ...
    it('swap0For1 to tick -1k with intermediate liquidity', async () => {
      ...
      await expect(pair.swap0For1(amount0In, walletAddress, '0x'))
        .to.emit(token1, 'Transfer')
        .withArgs(pair.address, walletAddress, '95299696652496117')

      expect(await pair.tickCurrent()).to.eq(-925)
    })
    ...
    it('swap1For0 to tick 1k', async () => {
      ...
      await expect(pair.swap1For0(amount1In, walletAddress, '0x'))
        .to.emit(token0, 'Transfer')
        .withArgs(pair.address, walletAddress, '94965947516311848')
    ...
    it('swap1For0 to tick 1k with intermediate liquidity', async () => {
      ...
      await expect(pair.swap1For0(amount1In, walletAddress, '0x'))
        .to.emit(token0, 'Transfer')
        .withArgs(pair.address, walletAddress, '95299696652496118')

      expect(await pair.tickCurrent()).to.eq(924)
    })
  })
  ...
  describe('limit orders', () => {
    ...
    it('selling 1 for 0 at tick 0 thru 1', async () => {
      await expect(pair.setPosition(0, 100, expandTo18Decimals(1)))
        .to.emit(token0, 'Transfer')
        .withArgs(walletAddress, pair.address, '4987272070749097')
      // somebody takes the limit order
      await pair.swap1For0(expandTo18Decimals(2), otherAddress, '0x')
      await expect(pair.setPosition(0, 100, expandTo18Decimals(1).mul(-1)))
        .to.emit(token1, 'Transfer')
        .withArgs(pair.address, walletAddress, '5027351678085459')
    })
    it('selling 0 for 1 at tick 0 thru -1', async () => {
      await expect(pair.setPosition(-100, 0, expandTo18Decimals(1)))
        .to.emit(token1, 'Transfer')
        .withArgs(walletAddress, pair.address, '4987272070749097')
      // somebody takes the limit order
      await pair.swap0For1(expandTo18Decimals(2), otherAddress, '0x')
      await expect(pair.setPosition(-100, 0, expandTo18Decimals(1).mul(-1)))
        .to.emit(token0, 'Transfer')
        .withArgs(pair.address, walletAddress, '5027351678085459')
    })
    ...
    describe('fee is on', () => {
      ...
      it('selling 1 for 0 at tick 0 thru 1', async () => {
        ...
        await expect(pair.setPosition(0, 100, expandTo18Decimals(1).mul(-1)))
          .to.emit(token1, 'Transfer')
          .withArgs(pair.address, walletAddress, '5024838002246417')
      })
      it('selling 0 for 1 at tick 0 thru -1', async () => {
        await expect(pair.setPosition(-100, 0, expandTo18Decimals(1)))
          .to.emit(token1, 'Transfer')
    ...
    describe('tickSpacing = 12', () => {
      ...
      describe('post initialize', () => {
        ...
        it('swapping across gaps works in 1 for 0 direction', async () => {
          ...
          await expect(pair.setPosition(120000, 121200, liquidityAmount.div(-2)))
            .to.emit(token0, 'Transfer')
            .withArgs(pair.address, walletAddress, '15013729147755')
            .to.emit(token1, 'Transfer')
            .withArgs(pair.address, walletAddress, '501499999999993248')
        ...
        it('swapping across gaps works in 0 for 1 direction', async () => {
          ...
          await expect(pair.setPosition(-121200, -120000, liquidityAmount.div(-2)))
            .to.emit(token0, 'Transfer')
            .withArgs(pair.address, walletAddress, '501499999999993248')
            .to.emit(token1, 'Transfer')
            .withArgs(pair.address, walletAddress, '15013729147755')
