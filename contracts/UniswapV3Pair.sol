...
contract UniswapV3Pair is IUniswapV3Pair {
    ...
    uint256 public offset0X128;
    uint256 public offset1X128;

    // accumulated protocol fees
    uint128 public override feeToFees0;
    uint128 public override feeToFees1;
    ...
    function _updatePosition(
        ...
        if (feeTo != address(0)) {
            uint128 fee0 = feesOwed0 / 6;
            feesOwed0 -= fee0;
            feeToFees0 += fee0;

            uint128 fee1 = feesOwed1 / 6;
            feesOwed1 -= fee1;
            feeToFees1 += fee1;
        }
    ...
    function mint(
        ...
        if (amount0 > 0) require(balance0.add(amount0) <= IERC20(token0).balanceOf(address(this)), 'M0');
        if (amount1 > 0) require(balance1.add(amount1) <= IERC20(token1).balanceOf(address(this)), 'M1');
    ...
    struct SwapState {
        ...
        uint256 offsetSpecified;
        uint256 offsetCalculated;
    }
    ...
    function _swap(SwapParams memory params) private {
        ...
        bool zeroSpecified = zeroForOne == (params.amountSpecified > 0);
        bool crossed = false;
        ...
        while (state.amountSpecifiedRemaining != 0 && state.sqrtPriceX96 != params.sqrtPriceLimitX96) {
            ...
            if (state.sqrtPriceX96 == step.sqrtPriceNextX96) {
                ...
                if (step.initialized) {
                    ...
                    if (zeroForOne) require(step.tickNext > minTick && step.tickNext < maxTick, 'MIN');
                    ...
                    if (!crossed) {
                        ...
                        state.balanceCalculatedInitial = uint128(
                            zeroSpecified
                                ? IERC20(token1).balanceOf(address(this))
                                : IERC20(token0).balanceOf(address(this))
                        );
                        state.offsetSpecified = zeroSpecified ? offset0X128 : offset1X128;
                        state.offsetCalculated = zeroSpecified ? offset1X128 : offset0X128;
                    ...
                    uint256 balanceSpecifiedAfter =
                        state.balanceSpecifiedInitial.addi(params.amountSpecified.sub(state.amountSpecifiedRemaining));
                    uint256 balanceCalculatedAfter =
                        params.amountSpecified > 0
                            ? state.balanceCalculatedInitial - uint128(state.amountCalculated)
                            : state.balanceCalculatedInitial + uint128(state.amountCalculated);

                    // update offsets
                    state.offsetSpecified = SqrtPriceMath.getOffsetAfter(
                    ...
                    state.offsetCalculated = SqrtPriceMath.getOffsetAfter(
        ...
        if (crossed) {
            offset0X128 = zeroForOne ? state.offsetSpecified : state.offsetCalculated;
        ...
        uint256 balanceBefore =
            uint256(params.amountSpecified > 0 ? state.balanceSpecifiedInitial : state.balanceCalculatedInitial);
        if (balanceBefore == 0) {
