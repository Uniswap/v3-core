...
library SafeMathExtra {
    function addi(uint256 x, int256 y) internal pure returns (uint256 z) {
        z = y < 0 ? SafeMath.sub(x, uint256(-y), 'SafeMathExtra::addi: underflow') : SafeMath.add(x, uint256(y));
    }

    function subi(uint256 x, int256 y) internal pure returns (uint256 z) {
        z = y < 0 ? SafeMath.add(x, uint256(-y)) : SafeMath.sub(x, uint256(y), 'SafeMathExtra::subi: overflow');
    }
}
