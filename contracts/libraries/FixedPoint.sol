...
library FixedPoint {
    ...
    uint private constant Q56 = 2**56;
    ...
    function add(uq112x112 memory self, uq112x112 memory y) internal pure returns (uq112x112 memory) {
        uint224 z;
        require((z = self._x + y._x) >= self._x, 'ds-math-add-overflow');
        return uq112x112(z);
    }
    ...
    // reverts on overflow
    function mul112(uq112x112 memory self, uint y) internal pure returns (uq112x112 memory) {
        uint z;
        require(y == 0 || (z = uint(self._x) * y) / y == uint(self._x), "FixedPoint: MULTIPLICATION_OVERFLOW");
        require(z <= 2**224, "FixedPoint: MULTIPLICATION_OVERFLOW");
        return uq112x112(uint224(z));
    }
    ...
    function reciprocal(uq112x112 memory self) internal pure returns (uq112x112 memory) {
        return uq112x112(uint224(Q224 / uint(self._x)));
    }

    // square root of a UQ112x112
    // TODO: is this right?
    function sqrt(uq112x112 memory self) internal pure returns (uq112x112 memory) {
        return uq112x112(uint224(Math.sqrt(self._x) * Q56));
    }
}
